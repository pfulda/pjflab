...
#2D array of size 1000*300=(#ofexposuresettings)*(#ofgainsettings) global
DarkNoise = np.empty((999,300))
...
def PixelPoints (picList,M,N): #picList will be unraveled numpy array of intensity values
	...
	Avg_Array = np.empty(np.size(Array_PixID))


    for j in range(np.size(Array_PixID)):
        ...
        for i in range(NUM_IMAGES):
            arr=np.ravel(np.array(picList[i])) #each iteration chooses different intensity image
            IVal[i] = arr[PixID] #each element of IVal will get intensity values from same pixel per Array_PixID iteration
        Avg_IV = np.mean(IVal) #averages all NUM_IMAGES elements in IVal to a single value
        Avg_Array[j] = Avg_IV #new avgd intensity added to Avg_Array per PixID iterated through

    Tot_Avg = np.mean(Avg_Array) #Averages all 81 elements of Avg_Array (i.e. 81 pixels) for avg intensity over sensor of a given (M,N)

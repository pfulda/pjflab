...
def User_Init():
    #default settings
    exp = 4000 #exposure time is in us
    gain = 1 #measured in dB
    NUM_IMAGES = int(input("Please enter the number of images you would like: "))  # number of images to grab
    """
    TODO:
    Add an option to run indefinitly
    """
    E = input("Please enter the exposure time in micro seconds you would like: (D for default exposure time)")
    G = input("Please enter the gain in dB you would like: (D for default gain)")
    if E != 'D':
        exp = E
    if G != 'D':
        gain = G
    return exp, gain, NUM_IMAGES

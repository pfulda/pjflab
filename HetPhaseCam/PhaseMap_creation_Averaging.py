...
folder = 'C:/Users/localadmin/Documents/pjflab/HetPhaseCam/Phase_Maps/'
...
for i in range(NUM_IMAGES):
	if i % 8 == 0 and i > 0 and i < NUM_IMAGES - 4:  #MAKE SURE OVERLAP IS RIGHT... (%8 FOR NOVAK, %4 FOR 4P AND CARRE)
		...
		if c == 1:
			...
			plt.ylim([540,0])
			plt.pause(0.00001)
			name = folder_path + "/" + Folder_Name + '_single_PM.png' #it's an average of ten so not mega
			plt.savefig(name)
			plt.show()
			plt.clf()
		...
		if c == 1:
			...
			plt.ylim([540,0])
			plt.pause(0.00001)
			name = folder_path + "/" + Folder_Name + '_single_unwrapped_PM_mask.png'
			plt.savefig(name)
			plt.show()
			plt.clf()
		...
		if c == 1:
			...
			plt.ylim([540,0])
			plt.pause(0.00001)
			name = folder_path + "/" + Folder_Name + '_single_rewrapped_PM.png'
			plt.savefig(name)
			plt.show()
			plt.clf()
